...
import type { Product, TechnicalSheet, TechnicalSheetVersion } from '../../types'
...
const VERSION_LABEL: Record<TechnicalSheetVersion, string> = {
  preliminare: 'Versione 1 · Preliminare',
  finale: 'Versione 2 · Finale',
  piazzamento: 'Versione 3 · Piazzamento e taglio',
}
...
export function SheetPdfDocument({ product, sheet }: { product: Product; sheet: TechnicalSheet }) {
    ...
    <div id="sheet-print" aria-hidden>
      <header className="pdf-header">
        ...
        <div className="pdf-meta">
          <p>{VERSION_LABEL[sheet.versione]}</p>
          ...
          <table className="pdf-table">
            <thead>
              <tr>
                ...
                <th className="r">Costo unitario</th><th className="r">Costo capo</th>
              </tr>
            </thead>
            <tbody>
              {materiali.map((m) => {
      ...
      {/* Versioni Finale e Piazzamento: note della versione e documento allegato. */}
      {(sheet.noteVersione || sheet.pdfFile || sheet.scanAI) && (
        <section className="pdf-section pdf-avoid-break">
          ...
          {sheet.scanAI?.note && <p className="pdf-text">{sheet.scanAI.note}</p>}
        </section>
      )}
      ...
      <footer className="pdf-footer">
        Heemia · Scheda tecnica {sheet.nomeProdotto || product.nome} · {VERSION_LABEL[sheet.versione]}
      </footer>
    </div>,
    document.body,
  )
